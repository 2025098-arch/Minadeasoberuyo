...
class OchirunaLogic {
    ...
    _activateAbility(type) {
        ...
        if (type === "dash") {
            console.log(
                "⚡ [Logic] 犬の能力「ダッシュ」発動！ スピード1.5倍！",
            );
            p.currentStats.speed = Math.floor(p.currentStats.speed * 1.5);
        } else if (type === "roll") {
            console.log(
                "⚡ [Logic] りんごの能力「転がる」発動！ スピード2倍、ジャンプ不可！",
            );
    ...
    _checkCollisions(p) {
        ...
        for (let [tileId, tile] of this.tiles.entries()) {
            ...
            const collisionRadius =
                this.CONSTANTS.TILE_RADIUS * 0.85 +
                this.CONSTANTS.PLAYER_RADIUS;
